...
import { PACK_TERMS_V10, QUSTODIO_CITATIONS } from './packs-v10';
...
async function seed(client) {
  ...
  for (const [wave, rows, key] of [
    ...
    ['v10', PACK_TERMS_V10, 'packs_v10_seeded'],
  ]) {
  ...
  if (!(await flagged(client, 'qustodio_citations_done'))) {
    for (const [term, src] of Object.entries(QUSTODIO_CITATIONS)) {
      ...
      await client.query(
        ...
        [term, line, src.sourceUrl]
      );
    }
    await flagDone(client, 'qustodio_citations_done');
  }
}
